// ─── DEMO DATA — dev/demo mode ONLY (EXPO_PUBLIC_USE_MOCK_DATA=true) ──────────
//
// Beautiful, fully-populated, FAKE sample content for screenshots and local
// testing. It must NEVER be rendered in live/production mode. Every consumer
// imports these behind an `isDemoMode` guard (see lib/runtimeMode.ts); the
// scripts/check-no-demo-leak.mjs static check enforces that.
//
