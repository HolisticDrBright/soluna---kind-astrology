// Google geo provider: birth-city autocomplete, place → lat/lng, and a
// date-aware IANA timezone. This is what lets Soluna refuse to compute an exact
// chart from a free-typed city: coordinates + timezone come ONLY from a resolved
// place here, never from guesses.
//
// Env: GOOGLE_MAPS_API_KEY (backend secret — proxied via the `geo` Edge Function
...
export interface ResolvedPlace {
  ...
  timezone: string;
  /** Total offset (raw + DST) in seconds for the requested date. */
...
export function parseAutocomplete(json: any): PlaceSuggestion[] {
  if (!json || (json.status && json.status !== "OK" && json.status !== "ZERO_RESULTS")) {
    throw new Error(`places autocomplete: ${json?.status ?? "bad response"}`);
  }
...
export function parsePlaceDetails(json: any): { lat: number; lng: number; label: string } {
  if (!json || json.status !== "OK") throw new Error(`place details: ${json?.status ?? "bad response"}`);
...
/** Unix seconds at noon UTC for a YYYY-MM-DD (date-aware DST without a time). */
...
export async function autocomplete(query: string): Promise<PlaceSuggestion[]> {
  ...
  const url = `${PLACES_AUTOCOMPLETE}?input=${encodeURIComponent(q)}&types=(cities)&key=${key()}`;
...
export async function resolvePlace(placeId: string, date: string): Promise<ResolvedPlace> {
  const detailUrl =
    `${PLACE_DETAILS}?place_id=${encodeURIComponent(placeId)}&fields=geometry,name,formatted_address&key=${key()}`;
  ...
  const tzUrl = `${TIMEZONE}?location=${lat},${lng}&timestamp=${timestampForDate(date)}&key=${key()}`;
