/**
 * Google geo parsers — pure normalization of Places / Details / TimeZone
 * responses. No network. Run: `deno test supabase/functions/_shared/tests`.
 */
...
Deno.test("autocomplete bad status throws (no silent fallback)", () => {
  assertThrows(() => parseAutocomplete({ status: "REQUEST_DENIED" }));
});

Deno.test("place details extracts geometry and requires it", () => {
  ...
  assertThrows(() => parsePlaceDetails({ status: "NOT_FOUND" }));
});
...
Deno.test("timestampForDate is noon UTC of that date", () => {
  ...
  assert(timestampForDate("1990-06-22") > 0);
});
